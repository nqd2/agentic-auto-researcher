...
import { type ToolRunContext, toolToOpenAI } from "./Tool.js";
...
const SYSTEM = `You are AAR (Agentic Auto-Researcher). You work only inside the user's project directory.
You have tools to read/write files, run shell commands, search the web, optional browser (Playwright), vector memory (Qdrant), graph (Neo4j read), Redis, and write reports to .aar/reports/.
...
export async function runAgentTurn(
  ...
  const registry = buildToolRegistry(() => opt.env);
